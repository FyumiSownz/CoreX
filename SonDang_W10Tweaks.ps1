...
$Tasks = @(
    ...
    @{
        Name = "Debloat - Windows 10 SysPrep & Privacy (Made by SonDang)";
        Action = {
            Function Begin-SysPrep {
                Write-Log "Starting Sysprep Fixes..." "INFO"
            } 

            # Tao PSDrive de truy cap 'HKCR' tree, tranh loi neu da tao tu truoc
            if (-not (Get-PSDrive -Name HKCR -ErrorAction SilentlyContinue)) {
                New-PSDrive -Name HKCR -PSProvider Registry -Root HKEY_CLASSES_ROOT | Out-Null
            }

            Function Start-Debloat {
                Write-Log "Removing AppxPackages..." "INFO"
                
                # Danh sach ung dung duoc bao ve (Modified whitelist by SonDang)
            ...
            Function Remove-Keys {
                Write-Log "Removing bloatware registry keys..." "INFO"
                $Keys = @(
                ...
                ForEach ($Key in $Keys) { Remove-Item $Key -Recurse -ErrorAction SilentlyContinue }
                $script:ApplyCount++
            }
                    
            Function Protect-Privacy {
                Write-Log "Protecting Privacy & Disabling telemetry..." "INFO"
                
                $Advertising = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\AdvertisingInfo'
                If (Test-Path $Advertising) { Set-ItemProperty $Advertising -Name Enabled -Value 0 -ErrorAction SilentlyContinue }
                    
                $Search = 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Search'
                If (Test-Path $Search) { Set-ItemProperty $Search -Name AllowCortana -Value 0 -ErrorAction SilentlyContinue }
                    
                $Period1 = 'HKCU:\Software\Microsoft\Siuf'
                $Period2 = 'HKCU:\Software\Microsoft\Siuf\Rules'
                $Period3 = 'HKCU:\Software\Microsoft\Siuf\Rules\PeriodInNanoSeconds'
                If (!(Test-Path $Period3)) { 
                    mkdir $Period1 -ErrorAction SilentlyContinue | Out-Null
                    mkdir $Period2 -ErrorAction SilentlyContinue | Out-Null
                    mkdir $Period3 -ErrorAction SilentlyContinue | Out-Null
                    New-ItemProperty $Period3 -Name PeriodInNanoSeconds -Value 0 -ErrorAction SilentlyContinue | Out-Null
                }
                           
                $registryPath = "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent"
                If (!(Test-Path $registryPath)) {
                    Mkdir $registryPath -ErrorAction SilentlyContinue | Out-Null
                    New-ItemProperty $registryPath -Name DisableWindowsConsumerFeatures -Value 1 -ErrorAction SilentlyContinue | Out-Null
                }          
                
                $Holo = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Holographic'    
                If (Test-Path $Holo) { Set-ItemProperty $Holo -Name FirstRunSucceeded -Value 0 -ErrorAction SilentlyContinue }
                
                $Live = 'HKCU:\SOFTWARE\Policies\Microsoft\Windows\CurrentVersion\PushNotifications'    
                If (!(Test-Path $Live)) {
                    mkdir $Live -ErrorAction SilentlyContinue | Out-Null
                    New-ItemProperty $Live -Name NoTileApplicationNotification -Value 1 -ErrorAction SilentlyContinue | Out-Null
                }
                
                $DataCollection = 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\DataCollection'    
                If (Test-Path $DataCollection) { Set-ItemProperty $DataCollection -Name AllowTelemetry -Value 0 -ErrorAction SilentlyContinue }
                
                $People = 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced\People'
                If (Test-Path $People) { Set-ItemProperty $People -Name PeopleBand -Value 0 -ErrorAction SilentlyContinue }

                $Suggestions = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager'    
                If (Test-Path $Suggestions) { Set-ItemProperty $Suggestions -Name SystemPaneSuggestionsEnabled -Value 0 -ErrorAction SilentlyContinue }
                ...
                # NTUSER.DAT Optimization by SonDang
            ...
            # App fixes and Optimization by SonDang
            Function FixWhitelistedApps {
                Write-Log "Checking and fixing whitelisted apps..." "INFO"
                If(!(Get-AppxPackage -AllUsers | Select Microsoft.Paint3D, Microsoft.MSPaint, Microsoft.WindowsCalculator, Microsoft.WindowsStore, Microsoft.MicrosoftStickyNotes, Microsoft.WindowsSoundRecorder, Microsoft.Windows.Photos)) {
                    Get-AppxPackage -allusers Microsoft.Paint3D | Foreach {Add-AppxPackage -DisableDevelopmentMode -Register "$($_.InstallLocation)\AppXManifest.xml" -ErrorAction SilentlyContinue}
                    ...
                    Get-AppxPackage -allusers Microsoft.WindowsCalculator | Foreach {Add-AppxPackage -DisableDevelopmentMode -Register "$($_.InstallLocation)\AppXManifest.xml" -ErrorAction SilentlyContinue}
            ...
            Function CheckDMWService {
                If (Get-Service -Name dmwappushservice -ErrorAction SilentlyContinue | Where-Object {$_.StartType -eq "Disabled"}) { Set-Service -Name dmwappushservice -StartupType Automatic -ErrorAction SilentlyContinue }
                If (Get-Service -Name dmwappushservice -ErrorAction SilentlyContinue | Where-Object {$_.Status -eq "Stopped"}) { Start-Service -Name dmwappushservice -ErrorAction SilentlyContinue } 
            }

            Function CheckInstallService {
                If (Get-Service -Name InstallService -ErrorAction SilentlyContinue | Where-Object {$_.Status -eq "Stopped"}) {  
            ...
            Write-Log "Hoan thanh chay Windows 10 Debloat (By SonDang)." "APPLY"
        }
    },
    @{
